(function () {
  ...
  const script =
    ...
    document.querySelector('script[data-website-id][src*="analytics.js"]');
  ...
  const sendLivePing = () => {
    fetch("https://web-track-seven.vercel.app/api/live-user", {
      ...
      // Do not surface ping errors to host websites.
    });
  };

  sendLivePing();

  window.addEventListener("pagehide", sendLivePing);
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") sendLivePing();
  });
